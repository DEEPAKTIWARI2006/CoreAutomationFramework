...
public class ReportingImpl {
  ...
  public ExtentReports getExtentReports(String TestName) {
    ...
    if (extentReports == null) {
      ...
      ExtentHtmlReporter htmlReporter = new ExtentHtmlReporter(reportspath + "/"
          + "GaleAutomationReport_" + TestName + "_" + getCurrentTimeStamp() + ".html");
  ...
  /**
   * The below method is used to capture the screenshot and returns the path of the captured
   * screenshot
   */
  /**
   * @param driver
  ...
  public static String CaptureScreenShot(final WebDriver driver) {
    ...
    try {
      ...
      dest = "./Reporting/Screenshots/" + getCurrentTimeStamp() + "_ErrorScreenshot.png";
 ...
 * scenario.createNode(And.class, "he has a receipt").pass("pass"); scenario.createNode(When.class,
 * "he returns the microwave").pass("pass"); scenario.createNode(Then.class,
 * "Jeff should be refunded $100").fail("fail");
 */

/*
 * public static void ScenarioNode(String scenarioName){ logger.info(""); logger.info(
 * "|--------------------------------------------------------------------------------------------------------------|"
 * ); logger.info("|------     Running scenario  " + scenarioName + "    -----|"); logger.info(
 * "|--------------------------------------------------------------------------------------------------------------|"
 * ); logger.info("");
 * 
 * parentTest = extentReports.createTest("Running scenario " + scenarioName); }
 * 
 * public static void Stepnode(String StepName){ logger.info("Running Step  " + StepName); childTest
 * = parentTest.createNode("Running Step " + StepName); }
 * 
 * public static void LogScenarioStatus(String ScenarioStatus, String Message) {
 * 
 * if (ScenarioStatus.equalsIgnoreCase("Pass")) { logger.info(
 * "|--------------------------------------------------------------------------------------------------------------|"
 * ); logger.info("|-------     Scenario  " + Message + "      -------|"); logger.info(
 * "|--------------------------------------------------------------------------------------------------------------|"
 * ); parentTest.pass(Message); logger.info(""); logger.info(Message); } else{ logger.info(
 * "|--------------------------------------------------------------------------------------------------------------|"
 * ); logger.info("|-------     Scenario  " + Message + "      -------|"); logger.info(
 * "|--------------------------------------------------------------------------------------------------------------|"
 * ); parentTest.fail(Message); logger.info(""); logger.info(Message); } }
 * 
 * public static void LogStepStatus(String StepStatus, String Message) {
 * 
 * if (StepStatus.equalsIgnoreCase("Pass")){ childTest.log(Status.PASS, Message);
 * logger.info(Message); } else if (StepStatus.equalsIgnoreCase("Info")){ childTest.log(Status.INFO,
 * Message); logger.info(Message); } else if (StepStatus.equalsIgnoreCase("Error")){
 * childTest.log(Status.ERROR, Message + Reporting.CaptureScreenShot()); logger.info(Message); }
 * else if (StepStatus.equalsIgnoreCase("Fail")){ childTest.log(Status.FAIL, Message + "  " +
 * "<a href='" + Reporting.CaptureScreenShot() + "  " + "'>Error Link</a>"); logger.info(Message); }
 * }
 * 
 ...
 * 
 * 
 * try{ TakesScreenshot ts = (TakesScreenshot)driver; File source =
 * ts.getScreenshotAs(OutputType.FILE); String dest = System.getProperty("user.dir")
 * +"/Reporting/ErrorScreenshots/" + GetCurrentTimeStamp() + "_ErrorScreenshot.png"; File
 * destination = new File(dest); FileUtils.copyFile(source, destination); return dest;
 * }catch(Exception e){ logger.info("Something went wrong while capturing screenshot"); return ""; }
 * 
 * }
 */
